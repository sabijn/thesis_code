...
if __name__ == '__main__':
    ...
    parser.add_argument('--mutated_dataset', type=str, default=None)
    ...
    if args.mutated_dataset:
        with open(args.mutated_dataset, 'r') as f:
            sentences = [sen.strip('\n') for sen in f.readlines() if sen != '\n']
    else:
        sentences = language.test_corpus


    tokenized_dataset = [tokenizer(x)['input_ids'] for x in sentences]
    ...
    if args.model == 'babyberta':
        ...
        store_model_probs(all_token_probs, sentences, f'{args.output_dir}/log_token_probs_eval_{args.model}_{args.version}_{args.top_k}.txt')
        bert_prob_dict = get_model_prob_dict(f'{args.output_dir}/log_token_probs_eval_{args.model}_{args.version}_{args.top_k}.txt')
        lm_probs, all_tokens = get_probs(sentences, tokenized_dataset, tokenizer, bert_prob_dict)

        with open(f'{args.output_dir}/log_token_probs_eval_{args.model}_{args.version}_{args.top_k}.pkl', 'wb') as f:
            pickle.dump((lm_probs, all_tokens), f)


    print(f"LM-PPL {np.exp(-np.mean(lm_probs))}")
    
    with open(f'{args.output_dir}/log_results_{args.model}_{args.version}_{args.top_k}_subset.json', 'w') as f:
        json.dump({

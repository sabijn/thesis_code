...
from model_probs import get_model_mlm_probs, get_model_clm_probs
...
if __name__ == '__main__':
    ...
    if args.model == 'gpt2':
        all_token_probs = get_model_clm_probs(model, 
                                                dataset['test'], 
                                                device,
        ...
        all_token_probs = get_model_mlm_probs(model, 
    ...
    for (idx, value) in all_token_probs.items():
        store_model_probs(value, dataset, output_dir / f'token_probs_eval_{args.model}_{args.top_k}_layer_{idx}.txt')

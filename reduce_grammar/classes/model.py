...
class LanguageClassifier(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if inputs_embeds is None:
            inputs_embeds = self.create_inputs_embeds(input_ids.to(self.device))
        ...
        if isinstance(self.encoder, nn.LSTM):
            ...
            hidden = torch.zeros_like(inputs_embeds, device=self.device)
            
            for idx in range(inputs_embeds.shape[1]):
                masked_input = input_ids.clone()
                masked_input[:, idx] = self.mask_idx
                masked_embeds = self.create_inputs_embeds(masked_input.to(self.device))
                output = self.encoder(
                    masked_input,
        ...
        if self.is_binary:
            ...
            if mask_ids is None and input_lengths is not None:
                predictions = self.unpad_sequence(predictions, lengths=input_lengths)
    ...
    def unpad_sequence(self, tensor, lengths, concat=True):
        ...
        return concat_fn(tensor_list).to(self.device)

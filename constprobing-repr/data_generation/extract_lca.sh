...
for version in "${versions[@]}"; do
    for topk in "${topks[@]}"; do
        echo "Creating lca labels for topk=${topk} and version=${version}"
        python extract_lca_labels.py --model_type ${MODEL} \
            --version ${version} \
            --top_k ${topk} \
            --data_dir ${DATA_DIR}/${version}/test_trees_${version}_${topk}.txt \
            --text_toks ${OUTPUT_DIR}/${MODEL}/${version}/${topk}/train_text.txt \
            --rel_toks ${OUTPUT_DIR}/${MODEL}/${version}/${topk}/train_rel_toks.txt \
            --rel_labels ${OUTPUT_DIR}/${MODEL}/${version}/${topk}/train_rel_labels.txt \
            --model_path ${MODEL_DIR}/${version}/${topk}/ \
            --next \
            --shared_levels ${OUTPUT_DIR}/${MODEL}/${version}/${topk}/train_shared_levels.txt \
            --unary ${OUTPUT_DIR}/${MODEL}/${version}/${topk}/train_unaries.txt
    done
done